...
class Translator {
	 ...
	 */
	_detectLanguage() {
		const inMemory = window.localStorage ? localStorage.getItem(this.config.persistKey) : undefined;
		...
		if (this._env === "browser") {
			...
			return new Promise((resolve) => {
				...
				urls.forEach((url, index) => {
					try {
						const json = JSON.parse(require("fs").readFileSync(process.cwd() + url, "utf-8"));
