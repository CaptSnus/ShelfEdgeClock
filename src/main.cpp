...
int displayMode = 0; // 0 = clock, 1 = date, 2 = temperature, 3 = humidity, 4 =
                     // scoreboard, 5 = countdown, 6 = scroll
...
uint32_t prevTimeSecCore0 =
    0; // reference for previous time in core 0 (1 second)
uint32_t prevTimeSecCore1 =
    0;                     // reference for previous time in core 1 (1 second)
...
bool clockMinPassed =
    true; // flag for the color change of the clock face (1 minute)
bool clockHourPassed =
    true; // flag for the color change of the clock face (1 hour)
bool clockDayPassed =
    true; // flag for the color change of the clock face (1 day)
bool downMinPassed =
    true; // flag for the color change of the downlights (1 minute)
bool downHourPassed =
    true; // flag for the color change of the downlights (1 hour)
bool downDayPassed =
    true; // flag for the color change of the downlights (1 day)
...
int wifiGW1, wifiGW2, wifiGW3,
    wifiGW4;          // wifi client - gateway address in blocks
...
//  |- global
int gloBrightness =
    0; // setting of the brightness -> 0 = automatic, > 0 = manual
int gloColorFreq =
    0; // 0 = every second, 1 = every minute, 2 = every hour, 3 = every day
...
String briKNXPath =
    ""; // path to the brightness item in the home automation system
int briKNXmin,
    briKNXmax; // minimum and maximum values provided by the home automation
String temKNXPath =
    ""; // path to the temperature item in the home automation system
String humKNXPath =
    ""; // path to the humidity item in the home automation system
HttpClient client = HttpClient(wifi, knxAddr.c_str(), knxPort);
//  |- downlights
int dowColorSet = 0; // 0 = 1 defined color, 1 = 14 defined colors , 2 = 1
                     // random color, 3 = 14 random colors
uint32_t dowColor;
...
String clkZone =
    "CET-1CEST,M3.5.0,M10.5.0/3"; // https://github.com/nayarsystems/posix_tz_db/blob/master/zones.csv
...
void initWifi() {
  ...
  //  access point
  apLocalIP =
      IPAddress(apIP1, apIP2, apIP3,
                apIP4); // populated and validated in web gui or predefinied
  apGateway =
      IPAddress(apGW1, apGW2, apGW3,
                apGW4); // populated and validated in web gui or predefinied
  apSubnet =
      IPAddress(apSN1, apSN2, apSN3,
                apSN4); // populated and validated in web gui or predefinied
  ...
  if (wifiDHCP) { // special settings for wifi client
    wifiLocalIP =
        IPAddress(wifiIP1, wifiIP2, wifiIP3, wifiIP4); // populated in web gui
    wifiGateway =
        IPAddress(wifiGW1, wifiGW2, wifiGW3, wifiGW4); // populated in web gui
    wifiSubnet =
        IPAddress(wifiSN1, wifiSN2, wifiSN3, wifiSN4); // populated in web gui
  }
  ...
  if (WiFi.status() !=
      WL_CONNECTED) {  // if no wifi connection after the wait period
...
void initNTP() {
  ...
  configTime(0, 0,
             clkAddr.c_str()); // First connect to NTP server, with 0 TZ offset
  setenv("TZ", clkZone.c_str(),
         1); // Overwrite system timezone with defined timezone
...
void initHandlers() {
  ...
  server.on("/getTemColorSet", [](AsyncWebServerRequest *request) {
    request->send(200, "text/plain", String(temColorSet));
  });
  server.on("/updTemColorSet", HTTP_POST, [](AsyncWebServerRequest *request) {
    temColorSet = request->arg("temColorSet").toInt();
    pref.putInt("temColorSet", temColorSet);
    request->send(200, "text/json", "{ \"result\":\"ok\" }");
  });
  server.on("/getTemAColor", [](AsyncWebServerRequest *request) {
    char tempcolor[8];
    sprintf(tempcolor, "#%02X%02X%02X", temAColorR, temAColorG, temAColorB);
    request->send(200, "text/plain", tempcolor);
  });
  server.on("/updTemAColor", HTTP_POST, [](AsyncWebServerRequest *request) {
    temAColorR = request->arg("r").toInt();
    temAColorG = request->arg("g").toInt();
    temAColorB = request->arg("b").toInt();
    pref.putInt("temAColorR", temAColorR);
    pref.putInt("temAColorG", temAColorG);
    pref.putInt("temAColorB", temAColorB);
    request->send(200, "text/json", "{ \"result\":\"ok\" }");
  });
  server.on("/getTemBColor", [](AsyncWebServerRequest *request) {
    char tempcolor[8];
    sprintf(tempcolor, "#%02X%02X%02X", temBColorR, temBColorG, temBColorB);
    request->send(200, "text/plain", tempcolor);
  });
  server.on("/updTemBColor", HTTP_POST, [](AsyncWebServerRequest *request) {
    temBColorR = request->arg("r").toInt();
    temBColorG = request->arg("g").toInt();
    temBColorB = request->arg("b").toInt();
    pref.putInt("temBColorR", temBColorR);
    pref.putInt("temBColorG", temBColorG);
    pref.putInt("temBColorB", temBColorB);
    request->send(200, "text/json", "{ \"result\":\"ok\" }");
  });
  server.on("/getTemCColor", [](AsyncWebServerRequest *request) {
    char tempcolor[8];
    sprintf(tempcolor, "#%02X%02X%02X", temCColorR, temCColorG, temCColorB);
    request->send(200, "text/plain", tempcolor);
  });
  server.on("/updTemCColor", HTTP_POST, [](AsyncWebServerRequest *request) {
    temCColorR = request->arg("r").toInt();
    temCColorG = request->arg("g").toInt();
    temCColorB = request->arg("b").toInt();
    pref.putInt("temCColorR", temCColorR);
    pref.putInt("temCColorG", temCColorG);
    pref.putInt("temCColorB", temCColorB);
    request->send(200, "text/json", "{ \"result\":\"ok\" }");
  });
  //  |- humidity
...
//  MAIN FUNCTIONS
//  ----------------------------------------------------------------------------------------------------
//  These functions are literally the content shown on the front side of the
//  shelf: modeClk:  current local time according to the timezone settings
...
void modeClk() {    // display the current time
  ...
  if (!clkFormat) {
    if (hour > 12) {
      hour = hour - 12;
    } // if hours is above 12, turn it from 13:mm into 01:mm PM
    if (hour < 1) {
      hour = hour + 12;
    } // if hours is 00:mm, turn it from 00:mm into 12:mm AM
  }
  ...
  if (clkColorSet == 1 &&
      ( // two random colors: one for hours, one for minutes
          (gloColorFreq == 0) || // change color every second
          (gloColorFreq == 1 &&
           clockMinPassed) || // change color every minute and 1 minute passed
          (gloColorFreq == 2 &&
           clockHourPassed) || // change color every hour and 1 hour passed
          (gloColorFreq == 3 &&
           clockDayPassed))) { // change color every day and 1 day passed
...
void modeTem() {    // display the temperature
  ...
  // set the colors
  if (temColorSet == 0) { // three defined colors: one for degrees, one for
                          // decimal and one for symbol
  ...
  if (temColorSet == 1 &&
      ( // three random colors: one for degrees, one for decimal and one for
        // symbol
          (gloColorFreq == 0) || // change color every second
          (gloColorFreq == 1 &&
           clockMinPassed) || // change color every minute and 1 minute passed
...
void modeHum() {    // display the humidity
  ...
  // set the colors
  if (humColorSet == 0) { // three defined colors: one for percents, one for
                          // decimal and one for symbol
  ...
  if (humColorSet == 1 &&
      ( // three random colors: one for percents, one for decimal and one for
        // symbol
          (gloColorFreq == 0) || // change color every second
          (gloColorFreq == 1 &&
           clockMinPassed) || // change color every minute and 1 minute passed
...
void modeSco() {    // display the scoreboard
  ...
  // set the colors
  scoHColor =
      ledClock.Color(scoHColorR, scoHColorG, scoHColorB); // color for home team
  scoAColor =
      ledClock.Color(scoAColorR, scoAColorG, scoAColorB); // color for away team
...
void loop() {
  ...
  if ((currMillisCore1 - prevTimeSecCore1) >=
      1000) { // run everything inside here every second
